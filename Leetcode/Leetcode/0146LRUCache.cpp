...
public:
	...
	void put(int key, int value) {
		// erase the existing (key, old value) pair from m_list
		auto it = m_map.find(key);
		if (m_map.end() != it) {
			m_list.erase(it->second);
		}

		// for all scenarios, push front (key, value) pair
		m_list.push_front(make_pair(key, value));
		m_map[key] = m_list.begin();

		// if size exceeds capacity, trim the last pair in m_list, also erase the corresponding key in m_map
